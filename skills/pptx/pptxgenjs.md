...
| `angle` | number | 0-359 degrees | Direction the shadow falls |
...
2. **NEVER encode opacity in hex color strings** - use the `opacity` property instead
3. **Use `bullet: true`** - NEVER unicode symbols like "•"
4. **Use `breakLine: true`** between array items
5. **Each presentation needs fresh instance** - do not reuse `pptxgen()` objects
6. **NEVER reuse option objects across calls** - PptxGenJS mutates objects in-place
